...
import { aspectRatioList } from './core/components/AspectRatioIcon';
import { resetActions } from './core/resetFunctions/resetActions';
import AspectRatioList from './core/components/AspectRatioList';
...
const Crop: React.FC<ICrop> = ({
  ...
  useEffect(() => {
    if (degreesCanvasRef.current) {
      ...
      if (degreesCtxRef.current) canvasDrawDegrees(degreesCtxRef.current, cropStep.image.angle);
    }
  ...
  const drawDegrees = () => {
    if (degreesCtxRef.current) {
      canvasDrawDegrees(degreesCtxRef.current, cropStep.angle.angle);
    }
  };
  ...
  const handleShowAspectRatioPanel = () => {
    toggleAspectRatioPanel();
  };

  const handleHideAspectRatioPanel = () => {
    toggleAspectRatioPanel();
  };
  ...
  //Zoom Image Buttons
  const clickImageZoom = (isZoomIn: boolean) => {
    const crop = cropStep.crop;
    if (!activeActions.isCroping) {
      if (!activeActions.isZooming) {
        activeActions.isZooming = true;
  ...
  return (
    <div className={styles.root}>
      ...
      <div className={styles.bottom}>
        {isPanelVisible && (
          <AspectRatioList selected={selectedAspectRatio?.id} setAspectRatio={handleSetCropAspectRatio} hide={handleHideAspectRatioPanel} />
        )}
