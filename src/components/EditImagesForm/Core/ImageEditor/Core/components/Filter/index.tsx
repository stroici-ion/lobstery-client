...
const Filter: React.FC<IFilter> = ({
  ...
  const getPreviewImageData = () => {
    if (!canvasCtxPrevieFilterRef.current || !canvasRef.current || !imageRef.current) return;

    const previewCropStep = getNullObject();
    previewCropStep.parentSize = {
      width: 100,
      height: 100,
    };
    previewCropStep.Origin = {
      x: 50,
      y: 50,
    };

    if (historyCropState.cropAR > 1) {
      previewCropStep.crop.height = 100;
      previewCropStep.crop.y = 0;
      previewCropStep.crop.width = 100 * historyCropState.cropAR;
      previewCropStep.crop.x = previewCropStep.Origin.x - previewCropStep.crop.width;
    } else {
      previewCropStep.crop.width = 100;
      previewCropStep.crop.x = 0;
      previewCropStep.crop.height = 100 / historyCropState.cropAR;
      previewCropStep.crop.y = previewCropStep.Origin.y - previewCropStep.crop.height;
    }

    loadHistoryStepPosition(previewCropStep, historyCropState, true, false);
    ...
    canvasDrawImage(img, undefined, imageRef.current, previewCropStep, 0, canvasCtxPrevieFilterRef.current, true);

    setPreviewFilterData(canvasCtxPrevieFilterRef.current.getImageData(0, 0, 100, 100));
  };
