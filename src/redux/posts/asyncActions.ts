...
export const fetchPostsByUser = createAsyncThunk<{ count: number; posts: IPost[] }, Record<string, string>, { rejectValue: IFetchError }>(
...
export const fetchPosts = createAsyncThunk<{ count: number; posts: IPost[] }, Record<string, string | undefined>, { rejectValue: IFetchError }>(
  'posts/fetchPosts',
  async (params, { rejectWithValue }) => {
    try {
      const res = await $api.get<FetchPostsResponse>('api/posts/', { params });
...
export const fetchCreatePost = createAsyncThunk<IPost, { post: IPostEdit; images: IImage[] }, { rejectValue: IFetchError }>(
  'posts/fetchCreatePost',
  async ({ post, images }, { dispatch, rejectWithValue }) => {
    try {
      ...
      if (post.customAudience > 0) {
        // @ts-ignore
        data.custom_audience = post.customAudience;
      }
...
export const fetchLikePost = createAsyncThunk<{ id: number; likesInfo: ILikesInfo }, { postId: number; like: boolean }, { rejectValue: IFetchError }>(
  'posts/fetchLikePost',
  async ({ postId, like }, { rejectWithValue }) => {
    try {
...
export const fetchFavoritePost = createAsyncThunk<{ id: number; favorite: boolean }, number, { rejectValue: IFetchError }>(
