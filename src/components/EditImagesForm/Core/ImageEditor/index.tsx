...
import { RevertSvg, ZoomInSvg, ZoomOutSvg } from '../../../../icons/imageEditor';
import { IImage } from '../../../../models/IImage';
import styles from './styles.module.scss';
import { filtersList } from './Core/Controls/filtersList';
...
const ImageEditor2: React.FC<IImageEditor2> = ({ image, onSave }) => {
  ...
  useEffect(() => {
    ...
    if (!imageRef.current || !canvasMarkupRef.current || !originalImageCanvasRef.current || !optimizedImageCanvasRef.current) return;
    ...
    if (cvsCtxOptimzed.current) {
      cvsCtxOptimzed.current.drawImage(imageRef.current, 0, 0, optimizedImageCanvasRef.current.width, optimizedImageCanvasRef.current.height);
    ...
    if (cvsCtxOriginal.current) {
      cvsCtxOriginal.current.drawImage(imageRef.current, 0, 0, originalImageCanvasRef.current.width, originalImageCanvasRef.current.height);
  ...
  }, [isImageLoaded, imageRef.current, canvasMarkupRef.current, optimizedImageCanvasRef.current, originalImageCanvasRef.current]);
  ...
  const handleSetTab = (t: EnumTabs) => {
    // isFiltersCanvasUsed.current = false;
  ...
  const getImageToRender = () => {
    if (originalImageCanvasRef.current) if (wasAdjusted.current || wasFiltered.current) return originalImageCanvasRef.current;
    return;
  };
  ...
  const handleSaveImage = () => {
    ...
    loadHistoryStepPosition(cropStep, historyCropValue, true, true);
    ...
    <div className={styles.root}>
      ...
      <div className={styles.header}>
        <div className={styles.zoomTools}>
          <button className={classNames(styles.zoomTools__in, styles.active)} onClick={handleClickZoomIn}>
            <ZoomInSvg />
          </button>
          <button className={styles.zoomTools__out} onClick={handleClickZoomOut}>
            <ZoomOutSvg />
          </button>
          <div className={classNames(styles.zoomTools__poportions)}>1:1</div>
          <div className={styles.zoomTools__sizePercent}>100%</div>
          <div className={styles.zoomTools__decoration}></div>
          <button className={classNames(styles.zoomTools__reset, styles.active)}>Reset</button>
