...
const Adjustment: React.FC<IAdjustment> = ({
  ...
  const isStopApplyingAdjustments = useRef(false);

  const apllyAdjustments = () => {
    if (!isInitialized.current) return;
    if (historyAdjustmentState) {
      if (!compareAdjustments(adjustmentsRef.current, historyAdjustmentState)) {
  ...
  const addAdjustmentStateToHistory = () => {
    if (isStopApplyingAdjustments.current) {
      isStopApplyingAdjustments.current = false;
      apllyAdjustments();
    }
  ...
  useEffect(() => {
    if (!isStopApplyingAdjustments.current) apllyAdjustments();
  }, [adjustments]);
  ...
  const onChangeAdjustment = useCallback((id: number, value: number) => {
    switch (id) {
      ...
      case 8:
        ...
        isStopApplyingAdjustments.current = true;
        break;
    }
